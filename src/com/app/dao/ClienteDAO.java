...
public class ClienteDAO implements Operaciones {
    ...
    public String modificar(Object obj) {
        ...
        try {
            ...
            Cliente cli = (Cliente) obj;
            Connection cn = this.conexion.conectar();
            sql = "UPDATE clientes SET nit=?, nombreCliente=?, descripcionCliente=?,"
                    + "registro =?, direccionCliente =?, idDepartamento =?, idGiro =? "
                    + "WHERE idCliente =?";
            PreparedStatement pst = cn.prepareStatement(sql);
            pst.setString(1, cli.getNit());
            pst.setString(2, cli.getNombreCliente());
            pst.setString(3, cli.getDescripcionCliente());
            pst.setString(4, cli.getRegistro());
            pst.setString(5, cli.getDireccionCliente());
            pst.setInt(6, cli.getDepartamento().getIdDepartamento());
            pst.setInt(7, cli.getGiro().getIdGiro());
    ...
    public List<?> consultar() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                dep.setIdDepartamento(rs.getInt(7));

                Giro giro = new Giro();
                giro.setIdGiro(rs.getInt(8));

                lst.add(new Cliente(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), dep, giro));
            }
            rs.close();
            sentencia.close();
            cn.close();
